# -*- coding: utf-8 -*-
"""
1404-specific bridge pipeline: link payments to appointments by date + name + phone.

Bridges:
- payments_1404_full.xlsx (تاريخ پذيرش, شماره پرونده, نام بیمار, موبایل)
- appointments in data/inputs/history/1404/*.xlsx (تاريخ نوبت, نام بیمار, موبایل/تلفن)

Does NOT use شماره نوبت (unusable: mostly 0/null).

PowerShell commands to run:
  cd C:\\Users\\USER\\Documents\\GitHub\\atieh
  python scripts/bridge_1404_payment_appointment.py

Or from any directory:
  python "<repo_path>/scripts/bridge_1404_payment_appointment.py"
"""
...
# Column candidates
PAYMENT_DATE_HEADERS = ["تاريخ پذيرش", "تاریخ پذیرش", "تاريخ", "تاریخ"]
PAYMENT_NAME_HEADERS = ["نام بيمار", "نام بیمار", "نام بیمار(تشکیل پرونده شده)"]
PAYMENT_PHONE_HEADERS = ["موبايل", "موبایل", "تلفن", "شماره تماس"]
PAYMENT_RECORDNO_HEADERS = ["شماره پرونده", "کد پرونده", "record_no", "پرونده"]

APPT_DATE_HEADERS = ["تاريخ نوبت", "تاریخ نوبت", "تاریخ", "تاريخ"]
APPT_NAME_HEADERS = [
...
def normalize_name(s) -> str:
    """Strip trailing (record_no), unify ي/ی and ك/ک, collapse spaces."""
    ...
    if t in ("", "nan", "None"):
        return ""
    t = re.sub(r"\s*\(\d+\)\s*$", "", t)
    t = re.sub(r"\s*\(تشکیل پرونده شده\)\s*$", "", t, flags=re.IGNORECASE)
    t = t.replace("ي", "ی").replace("ك", "ک").strip()
    t = " ".join(t.split())
    return t
...
def normalize_phones(raw: str | None) -> set[str]:
    """
    Split semicolon-separated phones; normalize to 11-digit 09XXXXXXXXX.
    +98 / 9XXXXXXXXX / 09XXXXXXXXX -> 09XXXXXXXXXX
    Converts Persian/Arabic digits to English for consistent matching.
    """
    out = set()
...
def run_bridge(
    ...
    def _add_match(m: dict, method: str, conf: float) -> None:
        ...
        if appt_idx is not None and (rn, appt_idx) not in seen_bridge:
            ...
            m["confidence"] = conf
            first_phone = (m.get("appointment_phone") or "").strip()
...
def ensure_schema(conn: sqlite3.Connection) -> None:
    conn.executescript("""
        DROP TABLE IF EXISTS bridge_1404_payment_appointment;
        CREATE TABLE bridge_1404_payment_appointment (
        ...
        CREATE INDEX IF NOT EXISTS idx_bridge_1404_record_no ON bridge_1404_payment_appointment(record_no);
        CREATE INDEX IF NOT EXISTS idx_bridge_1404_match_method ON bridge_1404_payment_appointment(match_method);
        CREATE INDEX IF NOT EXISTS idx_bridge_1404_appt_patient_key ON bridge_1404_payment_appointment(appointment_patient_key);

        DROP TABLE IF EXISTS bridge_1404_review;
        CREATE TABLE bridge_1404_review (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            record_no TEXT,
            payment_name_norm TEXT,
        ...
        CREATE INDEX IF NOT EXISTS idx_bridge_1404_review_record_no ON bridge_1404_review(record_no);

        DROP TABLE IF EXISTS patient_recordno_map_1404;
        CREATE TABLE patient_recordno_map_1404 (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        CREATE INDEX IF NOT EXISTS idx_patient_recordno_map_1404_record_no ON patient_recordno_map_1404(record_no);
    """)
    conn.commit()
...
def save_to_sqlite(
    ...
    for m in accepted:
        cur.execute("""
            INSERT OR IGNORE INTO bridge_1404_payment_appointment
    ...
    for r in review:
        cur.execute("""
            INSERT INTO bridge_1404_review (record_no, payment_name_norm, review_reason, reason, candidate_count)
            VALUES (?,?,?,?,?)
    ...
    for pm in patient_map:
        cur.execute("""
            INSERT OR REPLACE INTO patient_recordno_map_1404
...
def print_stats(
    ...
    print()
    print("=" * 70)
    print("1404 BRIDGE PIPELINE RESULTS")
    print("=" * 70)
    ...
    print("  cd " + str(REPO))
    print("  python scripts/bridge_1404_payment_appointment.py")
    print()
    print("Or from any directory:")
    print(f'  python "{REPO / "scripts" / "bridge_1404_payment_appointment.py"}"')
    print()


def main() -> int:
    payments_path = REPO / "data" / "inputs" / "payments" / "payments_1404_full.xlsx"
    appt_path = REPO / "data" / "inputs" / "history" / "1404" / "appointments_1404.xlsx"
