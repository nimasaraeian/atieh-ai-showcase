# -*- coding: utf-8 -*-
"""
1402-specific bridge pipeline: link payments to appointments by date + name + phone.

Bridges:
- payments_1402_full.xlsx (ØªØ§Ø±ÙŠØ® Ù¾Ø°ÙŠØ±Ø´, Ø´Ù…Ø§Ø±Ù‡ Ù¾Ø±ÙˆÙ†Ø¯Ù‡, Ù†Ø§Ù… Ø¨ÛŒÙ…Ø§Ø±, Ù…ÙˆØ¨Ø§ÛŒÙ„)
- appointments in data/inputs/history/1402/*.xlsx (ØªØ§Ø±ÙŠØ® Ù†ÙˆØ¨Øª, Ù†Ø§Ù… Ø¨ÛŒÙ…Ø§Ø±, Ù…ÙˆØ¨Ø§ÛŒÙ„/ØªÙ„ÙÙ†)
...
Or from any directory:
  python "<repo_path>/scripts/bridge_1402_payment_appointment.py"
"""
...
def load_appointments(path: Path) -> tuple[pd.DataFrame, dict]:
    ...
    # fallback for 1402 appointment files
    if not c_name:
        preferred = [
            "نام بیمار(تشکیل پرونده شده)",
            "نام بیمار (تشکیل پرونده شده)",
...
def ensure_schema(conn: sqlite3.Connection) -> None:
    conn.executescript("""
        DROP TABLE IF EXISTS bridge_1402_payment_appointment;
        CREATE TABLE bridge_1402_payment_appointment (
        ...
        CREATE INDEX IF NOT EXISTS idx_bridge_1402_record_no ON bridge_1402_payment_appointment(record_no);
        CREATE INDEX IF NOT EXISTS idx_bridge_1402_match_method ON bridge_1402_payment_appointment(match_method);
        CREATE INDEX IF NOT EXISTS idx_bridge_1402_appt_patient_key ON bridge_1402_payment_appointment(appointment_patient_key);

        DROP TABLE IF EXISTS bridge_1402_review;
        CREATE TABLE bridge_1402_review (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            record_no TEXT,
            payment_name_norm TEXT,
        ...
        CREATE INDEX IF NOT EXISTS idx_bridge_1402_review_record_no ON bridge_1402_review(record_no);

        DROP TABLE IF EXISTS patient_recordno_map_1402;
        CREATE TABLE patient_recordno_map_1402 (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        CREATE INDEX IF NOT EXISTS idx_patient_recordno_map_1402_record_no ON patient_recordno_map_1402(record_no);
    """)
    conn.commit()
...
def save_to_sqlite(
    ...
    for m in accepted:
        cur.execute("""
            INSERT OR IGNORE INTO bridge_1402_payment_appointment
    ...
    for r in review:
        cur.execute("""
            INSERT INTO bridge_1402_review (record_no, payment_name_norm, review_reason, reason, candidate_count)
            VALUES (?,?,?,?,?)
    ...
    for pm in patient_map:
        cur.execute("""
            INSERT OR REPLACE INTO patient_recordno_map_1402
...
def print_stats(
    ...
    print()
    print("=" * 70)
    print("1402 BRIDGE PIPELINE RESULTS")
    print("=" * 70)
    ...
    print("  cd " + str(REPO))
    print("  python scripts/bridge_1402_payment_appointment.py")
    print()
    print("Or from any directory:")
    print(f'  python "{REPO / "scripts" / "bridge_1402_payment_appointment.py"}"')
    print()


def main() -> int:
    payments_path = REPO / "data" / "inputs" / "payments" / "payments_1402_full.xlsx"
    appt_path = REPO / "data" / "inputs" / "history" / "1402" / "نوبت_دهی_بیمارانی_که_حضور_پیدا_میکنند_1402.xlsx"

    # Try common appointment filenames
    appt_dir = REPO / "data" / "inputs" / "history" / "1402"
