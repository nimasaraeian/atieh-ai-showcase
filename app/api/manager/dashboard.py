...
def get_top_value_patients(limit: int = 20) -> Dict[str, Any]:
    """
    Top patients by lifetime payment. Manager-only fields.
    """
    conn = _get_conn()
    try:
        cur = conn.cursor()

        # Prefer v_financial_identity_profile (has patient_name_canonical, mobile_canonical)
        # Fallback to financial_identity_profile if view missing
